import osimport sysimport timeimport paramikofrom netmiko import ConnectHandler

테스트때문에 콘다 설치경로 바꿨음

절대경로

path = "./setting"dev_path = path + "/devinfo"backup_path = path + "/backup"ip_path = path + "/ip"route_path = path + "/route"dhcp_path = path + "/dhcp"
...
def select_dev() :os.makedirs(dev_path, exist_ok=True)device_list = os.listdir(dev_path)for i in range(len(device_list)) :device_list[i] = device_list[i].replace(".txt","")
...
def connect_dev() :dev_info = select_dev()
...
def connect_linux(device_info) :cli = paramiko.SSHClient()cli.set_missing_host_key_policy(paramiko.AutoAddPolicy())
...
def roc_ip_set(cli):ens = input("설정할 인터페이스를 입력하세요.(ex):")restart = "nmcli connection down"+ " "+ ens + " " + ";" + "nmcli connection up" + " "+ ensipsetting = input("설정할 아이피 주소를 입력하세요:")           #변경할 ip정보 입력fixsetting = input("설정할 프리픽스를 입력하세요(예:/16):")     #변경할 프리픽스 입력stdin, stdout, stderr = cli.exec_command("nmcli connection modify " + ens + " ipv4.addresses " + ipsetting + fixsetting)print("ip가 변경됩니다... 프로그램이 종료됩니다.")stdin, stdout, stderr = cli.exec_command(restart) #위에서 설정한 재부팅 명령어 실행sys.exit()

def roc_gate_set(cli):                                                # 2번을 선택했을시에 gw 정보 변경ens = input("설정할 인터페이스를 입력하세요.(ex):")restart = "nmcli connection down"+ " "+ ens + " " + ";" + "nmcli connection up" + " "+ ensgatesetting = input("설정할 게이트웨이 주소를 입력하세요:")              #변경할 gw정보 입력stdin, stdout, stderr = cli.exec_command("nmcli connection modify"+" "+ ens +" "+ "ipv4.gateway"+" "+ gatesetting) #변경명령어print("ip가 변경됩니다... 프로그램이 종료됩니다.")stdin, stdout, stderr = cli.exec_command(restart) #위에서 설정한 재부팅 명령어 실행sys.exit()



def roc_menu(device_info) :cli = connect_linux(device_info)if cli is None:print("리눅스 연결 실패로 메뉴를 종료합니다.")return
...
def connect_cisco(device_info):conn_info = device_info.copy()conn_info.pop("type", None)conn_info.pop("name", None)
...
def command_cisco(cli, command) :if isinstance(command, str):command = command.splitlines()output = cli.send_config_set(command)print(output)print("장비 설정 적용 완료!")return output
...
def nat_restore(cli):backup_list = os.listdir(backup_path)if not backup_list:print("백업 파일이 없습니다.")return
...
def ip_preset() :cmd = ""os.makedirs(ip_path, exist_ok=True)name = input("생성할 ip셋의 이름 : ")while True :cmd += input("인터페이스\t: ")cmd += ' ' + input("네트워크\t: ")cmd += ' ' + input("서브넷\t: ")cmd += '\n'if 'n' == input("계속? y/n") :break

with open("%s/%s.txt"%(ip_path,name), "w") as file :
    file.write(cmd)

def ip_setting(cli) :command = ""ip_list = os.listdir(ip_path)for i in range(len(ip_list)) :ip_list[i] = ip_list[i].replace(".txt","")print(ip_list)name = input("인터페이스 일괄 등록 ip 파일명을 입력하세요. : ")
...
def route_preset() :cmd = ""os.makedirs(route_path, exist_ok=True)name = input("생성할 route셋의 이름 : ")while True :cmd += input("목적지 네트워크\t: ")cmd += ' ' + input("서브넷\t: ")cmd += ' ' + input("게이트웨이\t: ")cmd += '\n'if 'n' == input("계속? y/n") :break

with open("%s/%s.txt"%(route_path,name), "w") as file :
    file.write(cmd)

def route_setting(cli) :command = ""route_list = os.listdir(route_path)for i in range(len(route_list)) :route_list[i] = route_list[i].replace(".txt","")print(route_list)name = input("라우팅 경로 일괄 등록 라우트 파일명을 입력하세요. : ")
...
def static_router_setting(cli) :ro_ip_str = input("ip대역 입력: ")ro_sub_str = input("서브넷마스크 입력: ")ro_gw_str = input("게이트웨이 입력: ")
...
def nat_setting(cli):nat_menu = {"1": nat_static,"2": nat_dynamic,"3": pat}
...
def nat_static(cli) :outside_port_input = input("외부 망 포트 번호(예: e0/0, e1/0) : ").strip()inside_port_input = input("내부 망 포트 번호(예: e2/0, e2/1) : ").strip()local_ip_input = input("로컬 IP : ").strip()global_ip_input = input("공인 IP : ").strip()
...
def nat_dynamic(cli) :outside_port_input = input("외부 망 포트 번호(예: e0/0, e1/0) : ")inside_port_input = input("내부 망 포트 번호(예: e2/0, e2/1) : ")
...
def pat(cli) :outside_port_input = input("외부 망 포트 번호(예: e0/0, e1/0) : ")inside_port_input = input("내부 망 포트 번호(예: e2/0, e2/1) : ")
...
def dhcp_setting(cli) :name = input("DHCP 풀 이름: ")network = input("네트워크 대역과 마스크: ")gateway = input("디폴트 라우터: ")dns = input("DNS 서버: ")exclude = input("예외 주소 대역: ")
...
def svi_setting(cli) :vlan_id = input("VLAN id 번호 입력: ")ip = input("SVI IP 주소 입력: ")subnet = input("서브넷 마스크 입력: ")
...
def ubt_show_menu():res = int(input("""실행할 작업의 번호 선택하세요.1. 방화벽 해제2. 패키지 업데이트3. 패키지 세팅(wget, vim, libstdc++, tar, gzip)4. 미니콘다 설치5. 주피터 노트북 실행 설정6. systempython, 파라미코 설치7. dhcp ip 자동할당   ???8. netplan ip 수동 변환 및 할당   ???9. netplan 디렉토리 yaml 파일 리스트 쥬피터에 갱신10. yaml 파일 내용 파일 생성11. ip a12. ip r13. 패키지 확인 및 설치0. 종료선택: """))return res
...
def ubt_netplan_list_meth(output):file = open("netplan_ls", "w", encoding = "utf-8")file.write(output)file.close()
...
def ubt_yaml_cat():for i in range(len(yaml)):print("%d. %s"%(i+1,yaml[i]))global y_numy_num = int(input("파일을 선택해주세요 : "))yaml_cat = "cat /etc/netplan/".join(yaml[y_num-1])

yaml 안에 있는 내용을 netplan 장비명, 한줄 띄고 아웃풋 입력 -> 네트워크 장치 정보 입력

def ubt_make_yaml_cat(output):file = open("netplan_cat", "w", encoding = "utf-8")data = yaml[y_num] +"\n"+ outputfile.write(data)file.close()

ip a 백업

def ubt_ip_a(output):file = open("ub_ip_a", "w", encoding = "utf-8")file.write(output)file.close()

#ip r 백업def ubt_ip_r(output):file = open("ub_ip_r", "w", encoding = "utf-8")file.write(output)file.close()

#패키지 확인, 설치def ubt_package_check_install(cli):cmd = input("확인 필요한 패키지명 입력: ")
...
[Service]Type=simpleEnvironment="PATH=/root/miniconda3/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"ExecStart=/root/miniconda3/bin/jupyter notebook --allow-root --ip=0.0.0.0 --port=8080 --no-browserRestart=alwaysRestartSec=10
...
def ubt_menu(device_info) :cli = connect_linux(device_info)
